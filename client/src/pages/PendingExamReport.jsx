...
export default function PendingExamReport() {
  const [form, setForm] = useState({
    student_id: '365',
    start_date: '2014-08-29T00:00:00Z',
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch('/api/proxy/pendingExamReport', {
        method: 'POST',
    ...
    <div style={{ padding: '28px 32px', maxWidth: '1200px' }}>
      <div style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
          ...
          <code style={{ fontSize: '13px', color: '#6b7280', fontFamily: 'monospace' }}>api.proctoru.com/api/pendingExamReport/</code>
        </div>
        <h1 style={{ fontSize: '22px', fontWeight: '700', color: '#111827', marginBottom: '4px' }}>Pending Exam Report</h1>
        <p style={{ fontSize: '13px', color: '#6b7280' }}>Returns pending exams for a student within a date range.</p>
      </div>
