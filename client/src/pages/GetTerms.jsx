...
export default function GetTerms() {
  ...
  async function handleSubmit(e) {
    ...
    setResult(null);
    try {
      const res = await fetch('/api/proxy/getTerms', {
        method: 'POST',
    ...
    <div style={{ padding: '28px 32px', maxWidth: '1200px' }}>
      <div style={pageHeader}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
          ...
          <code style={{ fontSize: '14px', color: '#6b7280', fontFamily: 'monospace' }}>/api/getInstitutionTermList</code>
        </div>
        <h1 style={{ fontSize: '22px', fontWeight: '700', color: '#111827', marginBottom: '4px' }}>Get Terms</h1>
        <p style={{ fontSize: '13px', color: '#6b7280' }}>
          Returns the list of institution terms. Use the <strong>term_id</strong> values from the response in <em>Get Exams</em> or <em>Create Exam</em>.
        </p>
      </div>
